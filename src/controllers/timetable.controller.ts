...
async function createTimetable(req: Request, res: Response, next: NextFunction) {
  try {
    const timetable: TimetableDocument = await TimetableService.createTimetable(req.body)

    sendResponse(res, StatusCodes.HTTP_STATUS_CREATED, 'Timetable created successfully', timetable)
  } catch(err){
    next(err)
  }
}

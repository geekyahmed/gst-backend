...
async function getLoggedInUserProfile(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const user: UserDocument = await UserService.getLoggedInUserProfile(Types.ObjectId(sub))
...
async function createUser(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    sendResponse(res, StatusCodes.HTTP_STATUS_CREATED, 'User created successfully', user)
  } catch(err){
    next(err)
  }
}
